此项目用于解决华为Matebook E 2022二合一笔记本刷入Linux后 CPU功耗异常锁死4W 频率锁死400Mhz卡炸天且修改任何PL1/PL2都无法生效的情况。
原理：通过持续对EC寄存器0x45地址写入0x01 解开ec底层CPU功耗限制 解除后即可使用throttled或者其他软件设置PL1/PL2功耗

> [!NOTE]
> **Windows 环境说明**：在 Windows 下无需操作，可能华为相关驱动已进行类似操作，直接解PL1/2就行。此方案仅针对 Linux 系统。
...
- **硬件损坏风险**：修改 EC 寄存器及解除功耗限制可能导致设备发热严重，长期高温可能损坏 CPU、主板或其他元器件。虽然MBE内置风扇，但是最好长时间使用设置15w以下功耗，如果需要长时间20w请施加外部散热并且持续监测温度。
- **电池寿命衰减**：更高的功耗会加速电池老化，同时由于PKG功耗变高导致续航尿崩。
